...
import {
  LayoutDashboard, Package, TrendingUp, Trophy,
  Star, Upload, Zap, LogOut,
} from 'lucide-react'
    ...
    <aside className="w-60 flex-shrink-0 bg-slate-900 border-r border-slate-800 flex flex-col h-screen sticky top-0">
      ...
      <div className="px-3 py-4 border-t border-slate-800 space-y-1">
        <button
          ...
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-slate-500 hover:text-red-400 hover:bg-red-400/10 transition-all"
        >
          <LogOut className="w-4 h-4 flex-shrink-0" />
          로그아웃
        </button>
        <p className="text-xs text-slate-700 px-3">v1.0 · AX혁신팀</p>
      </div>
    </aside>
  )
}
